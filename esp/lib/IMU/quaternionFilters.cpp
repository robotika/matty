...
const float GyroMeasError = 3.0f; // Error 3 degrees/s
const float beta = sqrt(3.0f / 4.0f) * GyroMeasError * DEG_TO_RAD;
...
void MadgwickAHRSupdate(float g[3], float a[3], float m[3], float dT) {
  MadgwickAHRSupdate(g[0], g[1], g[2], a[0], a[1], a[2], m[0], m[1], m[2], dT);
}
...
void MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float dT) {
  ...
  if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
    ...
    recipNorm = 1.0f / (float) sqrtf(ax * ax + ay * ay + az * az);
    ax *= recipNorm;
    ay *= recipNorm;
    az *= recipNorm;

    // Normalise magnetometer measurement
    recipNorm = 1.0f / (float) sqrtf(mx * mx + my * my + mz * mz);
    mx *= recipNorm;
    my *= recipNorm;
    mz *= recipNorm;
    ...
    recipNorm = 1.0f / (float) sqrtf(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalise step magnitude
    s0 *= recipNorm;
    s1 *= recipNorm;
    s2 *= recipNorm;
    s3 *= recipNorm;
  ...
  recipNorm = 1.0f / (float) sqrtf(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3);
  q0 *= recipNorm;
  q1 *= recipNorm;
  q2 *= recipNorm;
  q3 *= recipNorm;
